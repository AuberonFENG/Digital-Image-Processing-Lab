...
int PlusDilationUnion1(int* visited, unsigned char *tempin, int Width, int Height, int row, int col, int index);
int PlusDilationUnion(int* visited, unsigned char *tempin, int Width, int Height, int row, int col, int index);
void CheckConnect1(Image*, char*);
void CheckConnect(Image*,char*);

int PlusDilationUnion2(int* visited, unsigned char *tempin, int Width, int Height, int row, int col, int index, unsigned char *tempout,unsigned char *tempout1);
...
Image *CannyEdgeDetection(Image*, double th1, double th2);
...
int TestReadImage(char *filename, char *outfilename)
{
	...
	/*
	// task1
	image = ReadPNMImage("D://DIP//DIP//PGM_IMAGES//PGM_IMAGES//large_septagon_gaussian_noise_mean_0_std_50_added.pgm");
	outimage = thresholding(image, otsuThresholding(image));
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//large_septagon_gaussian_noise_mean_0_std_50_added_OTSU.pgm");
	...
	outimage = thresholding(outimage, otsuThresholding(outimage));
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//large_septagon_gaussian_noise_mean_0_std_50_added_avg5_OTSU.pgm");
	image = ReadPNMImage("D://DIP//DIP//PGM_IMAGES//PGM_IMAGES//large_septagon_gaussian_noise_mean_0_std_50_added.pgm");
	outimage = thresholding(image, otsuThresholding1(image));
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//large_septagon_gaussian_noise_mean_0_std_50_added_OTSU1.pgm");
	outimage = AvgImage(image, 5);
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//large_septagon_gaussian_noise_mean_0_std_50_added_avg5.pgm");
	outimage = thresholding(outimage, otsuThresholding1(outimage));
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//large_septagon_gaussian_noise_mean_0_std_50_added_avg5_OTSU1.pgm");
	*/
	
	// task2
	image = ReadPNMImage("D://DIP//DIP//PGM_IMAGES//PGM_IMAGES//septagon_noisy_shaded.pgm");
	outimage = PartitionOtsuThresholding(image);
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//septagon_noisy_shaded_partition_otsu.pgm");
	
	/*
	//task3
	image = ReadPNMImage("D://DIP//DIP//PGM_IMAGES//PGM_IMAGES//spot_shaded_text_image.pgm");
	outimage = movingAverage(image);
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//spot_shaded_text_image_movingavg.pgm");
	*/
	/*
	//task4
	image = ReadPNMImage("D://DIP//DIP//PGM_IMAGES//PGM_IMAGES//defective_weld.pgm");
	outimage = regionGrowing(image);
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//defective_weld_regionGrowing.pgm");
	image = ReadPNMImage("D://DIP//DIP//PGM_IMAGES//PGM_IMAGES//noisy_region.pgm");
	
	int inputpos[5] = { 1, 3, 4, 5, 7 };
	int inputpos2[5] = { 0, 3, 6, 7, 8 };
	int inputpos3[13] = { 2,6,7,8,10,11,12,13,14,16,17,18,22 };
	//outimage = OpeningImage(image, 5, inputpos3, 13, 5, inputpos3, 13);
	//outimage = ClosingImage(outimage, 3, inputpos3, 13, 3, inputpos3, 13);
	
	outimage = ErosionImage(image, 5, inputpos3, 13);
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//noisy_region_opening.pgm");
	outimage = regionGrowing(outimage);
	SavePNMImage(outimage, "D://DIP//DIP//Lab10//Results//noisy_region_regionGrowing.pgm");
	*/
	return(0);
}
/*
...
int PlusDilationUnion2(int* visited, unsigned char *tempin, int Width, int Height, int row, int col, int index, unsigned char *tempout,unsigned char *tempout1){
	...
	for (int j = 0; j < 8; j++){
		...
		if ((m >= 0) && (m<Width) && (n >= 0) && (n < Height) && (tempin[pos] == 0) && (visited[pos] == 0) && (row >= 0) && (col >= 0) && (tempout[pos] != 255)){
			tempout1[pos] = (unsigned char)(255);
			printf("m:%d n:%d\n",m,n);
...
Image *regionGrowing(Image* image){
	...
	unsigned char* tempseed, *tempin,*tempout,*tempout1;
	tempin = image->data;
	...
	for (i = 0; i<size; i++)
	{
		*(tempout+i) = ((int)(*(tempout+i))<68) ? (unsigned char)(0) : (unsigned char)(255);
...
Image* movingAverage(Image* image) {
	...
	unsigned char* T = (unsigned char*)malloc(width * height * sizeof(unsigned char));
	Image* outimage;
	outimage = CreateNewImage(image, "#testing Swap");
	tempin = image->data;
	tempout = outimage->data;
...
Image *PartitionOtsuThresholding(Image* image){
	...
	for (m = 0; m < 3; m++){
		for (n = 0; n < 2; n++){
			...
			for (k = 0; k < partHeight; k++){
				for (t = 0; t < partWidth; t++){
					*(tempout + Width*(i + k) + (j + t)) = ((int)(*(tempin + Width*(i + k) + (j + t)))>threshold[m + 3 * n]) ? (unsigned char)(255) : (unsigned char)(0);
...
Image *CannyEdgeDetection(Image* image, double th1, double th2){
	...
	unsigned char* tempfilter;

	tempfilter = (unsigned char*)malloc(3 * 3 * sizeof(unsigned char));
	double* tempangle;
...
Image *LoGImage(Image* image,  int ksize, double sigma, double th){
	...
	tempfilter = (unsigned char*)malloc(ksize*ksize * sizeof(unsigned char));
...
Image *BoundaryExtraction(Image* image, int f, int* pos, int p_size){
	unsigned char *tempin, *tempout, *tempEro;
	int i, size;
...
Image *ErosionImage(Image *image, int f, int* pos, int p_size){ // min value
	...
	*/
	// define the temp filter to save a f*f filter data
	unsigned char* tempfilter;
	tempfilter = (unsigned char*)malloc(f*f*sizeof(unsigned char));
...
Image *InverseMagnitudeDFT_norm_exp(DFT* inputdft, Image *image){
	...
	outimage->data = (unsigned char*)malloc(outimage->Height*outimage->Width*sizeof(unsigned char));
	outimage->Type = GRAY;
	tempout = outimage->data;
	int inputW = inputdft->Width;
	int inputH = inputdft->Height;
	...
	for (i = 0; i < inputH; i++){
		for (j = 0; j < inputW; j++){
			...
			*(tempout + index) = (unsigned char)(*(temp + index));
...
Image *thresholding(Image* image, int th){
	...
	for (i = 0; i<size; i++)
	{
		...
		*tempout = ((int)(*tempin)>th) ? (unsigned char)(255) : (unsigned char)(0);
...
Image *LocalHistogram(Image* image){
	...
	for (i = 1; i < inputH - 1; ++i){
		for (j = 1; j < inputW - 1; ++j){
			...
			tempout[inputW*i + j] = (unsigned char)(cdf[(int)(*(tempfilter + 4))]);
		}
	}
...
Image *GlobalHistogram(Image* image){
	...
	for (i = 0; i < size; i++){
		*(tempout + i) = (unsigned char)(cdf[(int)(*(tempin + i))]);
	}
	return(outimage);
}
...
Image *SharpenImage(Image* mask, Image* image, float C){
	unsigned char *tempin, *tempout, *tempmask;
	int i, j,k,size;
	i = 0;
	...
	for (int k=0;k<size;k++){
		...
		*(tempout+k) = (unsigned char)(map((float)((*tempin+k)+C*(*tempmask+k)),min,max,0.0,255.0));
...
Image *TranslateImage(Image *image, int dx, int dy){// dx>0:move right, dy>0:move down
	...
	outimage->data = (unsigned char*)malloc(5*size);// if given exactly size of the output image, when dx>0 and dy>0, it will oversize this array.
	tempin = image->data;
	tempout = outimage->data;
	// first set default canvas background as black color
	for (i = 0; i<outputW*outputH; i++)
...
Image *GeoAvgImage(Image *image, int f){
	...
	for (j = Width*(f - 1) / 2 + (f - 1) / 2; j <= (Width*Height - 1) - (Width*(f - 1) / 2 + (f - 1) / 2); j++){
		if (m>Width - f + 1){// m and n is used to jump over the boundary data
			...
			*tempout = (unsigned char)(tempavg+0.5);
			//printf("%d\n", (int)(*tempout));
		}
		tempout++;
		tempin++;
